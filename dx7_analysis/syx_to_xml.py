...
# ***
# Run the script with:
# 1) python syx_to_xml.py filename.syx
# It will create a .xml file with the same name.
# OR
# 2) define a new filename for the xml:
# python syx_to_xml.py filename.syx new_filename.xml
# OR
# 3) for a directory of SysEx files (MOST USEFUL):
# python syx_to_xml.py foldername

# Easiest way is to have the syx directory in the same root as this app.
# ***

# Parses a single 128-byte DX7 patch and extracts its parameters into a dictionary
def parse_dx7_patch(data):
    ...
    # Operator-specific parameters: 6 operators, each with 17 bytes of data.
    for op in range(6):
        base = op * 17
        ...
        patch[f"operator_{6 - op}"] = { # Reverse order: operator_6 is first.
...
# Function to convert a DX7 SysEx file to XML format.
def syx_to_xml(syx_file, xml_file):
    with open(syx_file, "rb") as f:
        full_syx_data = f.read()

    # Checks if the header is in correct SysEx form.
    ...
    # Loops through each patch in the SysEx data and parses it.
    ...
    # Outputs XML with pretty-print formatting.
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("For one file:")
        print("python syx_to_xml.py filename.syx [output.xml]")
        print("OR")
    ...
    if os.path.isdir(input_path):
        ...
        # Handles a single file conversion.
