...
export default function generateSyx(patch) {
    ...
    for (let index = 0; index < 32; index++) {
        ...
        if (index !== 0) {
            currentPatch = {
                name: `Empty${index}`,
                algorithm: Math.random(Math.floor(1, 32)),
                feedback: Math.random(Math.floor(1, 7)),
                oscillator_sync: Math.random(Math.floor(0, 1)),
                lfo_speed: 35,
                lfo_delay: 0,
                lfo_pm_depth: 0,
                lfo_am_depth: 0,
                ...
                lfo_sync: 0,
                transpose: 24,
                pitch_eg_rate1: Math.random(Math.floor(80, 99)),
                pitch_eg_rate2: Math.random(Math.floor(60, 99)),
                pitch_eg_rate3: Math.random(Math.floor(90, 99)),
                pitch_eg_rate4: Math.random(Math.floor(60, 99)),
                pitch_eg_level1: 50,
                pitch_eg_level2: 50,
                pitch_eg_level3: 50,
                pitch_eg_level4: 50,
                operatorParams: Array.from({ length: 6 }, () => ({
                    ...
                    eg_level3: 0,
                    eg_level4: 0,
                    key_scaling_break: Math.random(Math.floor(0, 99)),
                    key_scaling_left_depth: 0,
                    key_scaling_right_depth: 0,
                    key_scaling_left_curve: Math.random(Math.floor(0, 3)),
                    key_scaling_right_curve: Math.random(Math.floor(0, 3)),
                    oscillator_detune: 0,
                    rate_scaling: 0,
